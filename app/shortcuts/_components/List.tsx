...
function ShortcutCard({
  ...
  return (
    ...
    >
      <PopoverTrigger>
        <Card
          className={clsx("border-1 border-transparent", {
            "border-danger-500": !success,
          })}
        >
          <div
            className={clsx(
              "absolute bg-white opacity-30 blur-lg h-full w-full flex justify-center items-center",
              {
                hidden: !running,
              }
            )}
          ...
          <CardFooter>
            ...
            >
              <Spinner
                className={clsx("absolute z-40", { hidden: !running })}
              />
              {actionText}
            </CommonButton>
          </CardFooter>
...
function AutoStatCard({ creds, id }: { creds: Credentials; id: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (autoStat) {
        const command = autoStat.command as {
          stat: keyof Stats;
          status: boolean;
        };
      ...
      <PopoverTrigger>
        ...
        >
          ...
          <CardFooter className="flex justify-between gap-2 ">
            <Spinner
              className={clsx("absolute z-40 w-full", { hidden: !saving })}
            />
            <Dropdown>
    ...
    <div className="w-full flex flex-col justify-end gap-3">
      ...
      <ShortcutGroup title="Equipment">
        <ShortcutCard
          image={perScene}
          actionText="Equip"
          onClick={equipMaxPerceptionHandler}
        >
          Max Perception
        </ShortcutCard>
        <ShortcutCard
          image={strScene}
          actionText="Equip"
          onClick={equipMaxStrengthHandler}
        >
          Max Strength
        </ShortcutCard>
        <ShortcutCard
          image={intScene}
          actionText="Equip"
          onClick={equipMaxIntelligenceHandler}
        >
          Max Intelligence
        </ShortcutCard>
        <ShortcutCard
          image={conScene}
          actionText="Equip"
          onClick={equipMaxConstitutionHandler}
        >
          Max Constitution
        </ShortcutCard>
      </ShortcutGroup>
